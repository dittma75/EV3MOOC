...
class PreRegisterHandler(BaseHandler):
    """Handler for course pre-registration."""

    def get(self):
        """Handles GET request."""

        self.render('pre_register.html')

    def post(self):
        """Handles POST requests."""

        pr_name = self.request.get('form02')
        pr_email = self.request.get('form03')
		
        pr = PreRegisterEntity(name=pr_name, email=pr_email)
        if pr.check_for_email():
            self.redirect('/course#pre_registration_exists')
        else:
            pr.put()
            try:
                mail.send_mail(to=pr_email, sender='registration@cs4hsrobots.appspotmail.com',
                               subject='Preregistration Successful - Educational Robots for Absolute Beginners',
                               body=('Thank you for preregistering for the'
                               ' "Educational Robots for Absolute Beginners" online course, sponsored' 
                               ' by Google CS4HS. The course starts on November 15th, 2013.'
                               ' The course runs for a total of five weeks. On Friday, November 15th we will'
                               ' post our first week of materials and you will be able to get started with'
                               ' course at http://cs4hsrobots.appspot.com/ at any time of day (or night) you wish.'
                               " The following Friday we will post the next week's worth of materials, and so on."
                               ' While we strongly encourage you to complete each week of the course that week, we'
                               " understand that's not always possible, and so you will continue to have full"
                               ' access to earlier lessons as we post additional ones. You will find the answers'
                               ' to many of your questions at:  https://cs4hsrobots.appspot.com/faq\n\n'
            ...
            self.redirect('/course#pre_registration_confirmation')
...
class GetStartedHandler(BaseHandler):
    """Handler for faq page."""
    ...
    def get(self):
        ...
        self.template_value['navbar'] = {'getstarted': True}
        self.render('getstarted.html')
...
class CertHandler(BaseHandler):
    ...
    def get(self):
        ...
        self.template_value['overall_score'] = course.get_overall_score(student)
        self.template_value['score_list'] = course.get_all_scores(student)
        self.template_value['navbar'] = {'cert': True}
        self.template_value['student'] = student
        self.render('cert.html')